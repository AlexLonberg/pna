...
const options = {
  ...
  bool: [
    '--dev',
    // используйте любое имя псевдонима, включая имя свойства
    'w'
  ]
}
...
Экземпляр `Map`, где ключами являются все существующие параметры, включая псевдонимы, а значениями индекс массива `args`.
